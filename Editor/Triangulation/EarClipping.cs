    ...
    public static class EarClipping
    {
        public static List<Triangle> Triangulate(List<Vector2> points)
        {
            ...
            for (ushort i = 0; i < points.Count; i++)
                vertices.Add(new TriangleVertex { Position = points[i], Index = i });

            for (var i = 0; i < vertices.Count; i++)
            {
                vertices[i].PreviousVertex = vertices[Pinch(i - 1, vertices.Count)];
                vertices[i].FollowingVertex = vertices[Pinch(i + 1, vertices.Count)];
            }
